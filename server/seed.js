...
const run = async () => {
  ...
  console.log('🧹 Clearing Sprint 1 demo data...');
  await Promise.all([
    User.deleteMany({}),
  ...
  const mk = (name, email, role, district) => ({
    name,
    email,
    passwordHash: hash,
    role,
  ...
  const [admin, citizen1, citizen2, volunteer1] = await User.create([
    mk('Admin', 'admin@bachao.org', 'admin', 'Dhaka'),
    mk('Karim Uddin', 'karim@example.com', 'citizen', 'Sylhet'),
    mk('Rahima Begum', 'rahima@example.com', 'citizen', 'Sunamganj'),
    mk('Volunteer Hasan', 'hasan@example.com', 'volunteer', 'Sylhet'),
  ]);

  console.log('🆘 Creating Sprint 1 help requests...');
  await HelpRequest.create([
    {
      createdBy: citizen1._id,
      needType: 'food',
      description: 'Food needed for families affected by flood water.',
      peopleAffected: 12,
    ...
    },
    {
      ...
      needType: 'water',
      description: 'Safe drinking water is urgently needed.',
      peopleAffected: 8,
    ...
    },
    {
      createdBy: citizen1._id,
      needType: 'rescue',
      description: 'SOS — people are trapped near rising water.',
      peopleAffected: 4,
      location: { district: 'Feni', upazila: 'Sadar', coords: PLACES.Feni },
      urgency: 'sos',
    },
  ]);

  console.log('\\n✅ Sprint 1 seed complete!');
  console.log('Login password for all demo users: password123');
  console.log('  admin@bachao.org');
  console.log('  karim@example.com');
  console.log('  hasan@example.com');
